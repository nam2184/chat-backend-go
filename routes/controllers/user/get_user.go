...
func GetUser(w http.ResponseWriter, r *http.Request, db *sqlx.DB, opts *options.HandlerOptions) {
	...
	constraint, err := queries.NewConstraint("id = $%", id)
	if err != nil {
		opts.Problem.HandleError(middleware.NewError(w, r, err))
		return
	}
	...
	if len(q.Rows) == 0 {
		...
		returnedRow := q.Rows[0]
		userResponse200 := response.GetUser{
			ID:        returnedRow.ID,
			FirstName: returnedRow.FirstName,
			Surname:   returnedRow.Surname,
			Username:  returnedRow.Username,
			Email:     returnedRow.Email,
		}
		json_res, err = json.Marshal(userResponse200)
		if err != nil {
			opts.Problem.HandleError(middleware.NewError(w, r, err))
			return
		}
