...
func GetMessages(w http.ResponseWriter, r *http.Request, db *sqlx.DB, opts *options.HandlerOptions) {
	ctx := r.Context()
	...
	query_params = util.ValidateQueries[qm.Message](query_params)
	tx := db.MustBegin()
	...
	q, err := queries.SelectOffsetQuery[qm.Message](tx, limit, skip, sort_by, order, query_params, constraint, nil)
	if err != nil {
		opts.Problem.HandleError(middleware.NewError(w, r, err))
		return
	}
	...
	var json_res []byte
	preprocessedArray := make([]response.GetMessages, 0)
...
func encodeBase64(message qm.Message) response.GetMessages {
	var resp response.GetMessages
